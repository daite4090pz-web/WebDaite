...
def zapolnitSeed(engine):
    ...
    try:
        ...
        if not sessia.query(Category).first():
            kategorii = {}
            cats = [
                Category(nazvanie='Страны', opisanie='Государства мира'),
                Category(nazvanie='Механики', opisanie='Игровые механики'),
                Category(nazvanie='Юниты', opisanie='Боевые подразделения'),
                Category(nazvanie='Технологии', opisanie='Исследования'),
                Category(nazvanie='Национальные фокусы', opisanie='Уникальные фокусы'),
        ...
        # Страны с привязкой к категории "Страны"
        if not sessia.query(Country).first():
            strany = [
                Country(nazvanie='Германский Рейх', teg='GER', ideologiya='Фашизм',
                        istoricheskayaSpravka='Мощная военная промышленность.', flagFileName='ger.png'),
                Country(nazvanie='СССР', teg='SOV', ideologiya='Коммунизм',
                        istoricheskayaSpravka='Огромная армия и ресурсы.', flagFileName='sov.png'),
                Country(nazvanie='США', teg='USA', ideologiya='Демократия',
                        istoricheskayaSpravka='Сильная экономика.', flagFileName='usa.png'),
                Country(nazvanie='Великобритания', teg='ENG', ideologiya='Демократия',
                        istoricheskayaSpravka='Крупный флот.', flagFileName='eng.png'),
                Country(nazvanie='Япония', teg='JAP', ideologiya='Фашизм',
                        istoricheskayaSpravka='Агрессивная политика.', flagFileName='jap.png')
            ]
        ...
        # Юниты с привязкой к "Юниты"
        if not sessia.query(Unit).first():
            unity = [
                Unit(nazvanie='Пехота', tip='Пехота', ataka=20, zashita=25, skorost=4, opisanie='Основная пехота.'),
                Unit(nazvanie='Лёгкий танк', tip='Бронетехника', ataka=35, zashita=15, skorost=8, opisanie='Быстрые танки.'),
                Unit(nazvanie='Артиллерия', tip='Поддержка', ataka=55, zashita=5, skorost=3, opisanie='Огневая поддержка.')
            ]
...
class Statya(SqlAlchemyBase):
    ...
    category = orm.relationship('Category', secondary=statyaCategorySvyaz, backref='statyi')
